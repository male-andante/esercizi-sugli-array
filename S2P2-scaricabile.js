...
const SCONTOAMBASSADOR = 0.3
let riepilogoCarrello = (prices[0]+prices[1]+prices[2])
let riepilogoCarrelloAmbassador = (1-SCONTOAMBASSADOR)* riepilogoCarrello
let totale = 0
let utenteCheEffettuaLAcquisto = marco //cambia il valore qui per provare se il tuo algoritmo funziona!

for (let i = 0; i < utenti.length; i++){
  if (utenteCheEffettuaLAcquisto.isAmbassador && riepilogoCarrello > 100) {
    totale = riepilogoCarrelloAmbassador
  } else if (utenteCheEffettuaLAcquisto.isAmbassador && riepilogoCarrello <= 100) {
    totale = riepilogoCarrelloAmbassador + shippingCost
  } else if (!utenteCheEffettuaLAcquisto.isAmbassador && riepilogoCarrello > 100) {
    totale = riepilogoCarrello
  } else if (!utenteCheEffettuaLAcquisto.isAmbassador && riepilogoCarrello <= 100) {
